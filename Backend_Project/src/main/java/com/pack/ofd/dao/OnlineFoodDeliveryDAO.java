...
public class OnlineFoodDeliveryDAO {
	...
	public boolean validatingUser(String fromTable, String email, String pswd) throws SQLException {
		User user = new User();
		try {
			Class.forName("com.mysql.cj.jdbc.Driver");
			Connection con =DriverManager.getConnection("jdbc:mysql://localhost:3306/Online_Food_Delivery","root","root");
			Statement st = con.createStatement();
			String loginValidateQuery = "SELECT email_id, password FROM "+ fromTable +" WHERE email_id = '"
					+ email + "&& password ='" + pswd
					+ "'";
